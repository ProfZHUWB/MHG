...
def load(data_dir='../../dataset', drop_useless=True, sampling_cfg=(20000,20000,1,1)): 
    ...
    df['Year_of_Stop'] = pd.DatetimeIndex(df['Date Of Stop']).year  # 35  Year of Stop    1292399 non-null  int64         
    ...
    df[y_col] = df['Violation Type'].apply(lambda x: 0 if x=='Warning' else 1)
    df.drop(columns='Violation Type', inplace=True) 
    ...
    if drop_useless:
        ...
        # print(df.info())
        # 筛选前 1292398 条数据，筛选完后 1228388 条数据
        # # 分类任务是预测第24个变量，违章类型，也是2018年Similarity encoding for learning with dirty categorical variables这篇文章的预测任务
        # # 1. Date Of Stop，Time Of Stop，Year_of_Stop对预测违章类型帮助不大，删掉
        # # 2. Agency 是 unique value，删掉
        # # 3. Geolocation 与 Latitude 和 Longitude 重复，删掉 Geolocation
        df.drop(columns=['Date Of Stop','Time Of Stop','Agency','Geolocation','Year_of_Stop'], inplace=True) 
    ...
    #   Column          Non-Null Count    Dtype   
    # ---  ------          --------------    -----   
    #  0   SubAgency       1292389 non-null  category
    #  1   Description     1292390 non-null  category
    #  2   Location        1292397 non-null  category
    #  3   Latitude        1197045 non-null  float64 
    #  4   Longitude       1197045 non-null  float64 
    #  5   State           1292340 non-null  category
    #  6   VehicleType     1292399 non-null  category
    #  7   Year            1284325 non-null  float64
    #  8   Make            1292342 non-null  category
    #  9   Model           1292212 non-null  category
    #  10  Color           1276272 non-null  category
    #  11  Violation Type  1292399 non-null  category
    #  12  Charge          1292399 non-null  category
    #  13  Race            1292399 non-null  category
    #  14  Gender          1292399 non-null  category
    #  15  Driver City     1292182 non-null  category
    #  16  DL State        1291470 non-null  category
    #  17  Arrest Type     1292399 non-null  category
...
if __name__ == "__main__":
    df,y_col = load(drop_useless=True)
    print(df.info())