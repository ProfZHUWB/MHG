...
def load(data_dir="../../dataset", drop_useless=True, select_year=2014, sampling_cfg=None): # sampling_cfg=(20000,20000,1,1)):

    # 1. read/uncompress data
    filename = os.path.join(data_dir, 'LACity/Crime/Crime_Data_from_2010_to_Present.zip')
    with ZipFile(filename) as zip_file:
        with zip_file.open(r'Crime_Data_from_2010_to_Present.csv') as file:
    ...
    df['TIME OCC'] = df['TIME OCC'].apply(lambda x: int(x[:2])*60+int(x[-2:]))
    ...
    df['report_lag'] = df['report_lag'].apply(lambda x: x.days)
    ...
    df['Mocodes len'] = df['Mocodes'].apply(lambda x: len(str(x).split()))
    df['Mocodes first'] = df['Mocodes'].apply(lambda x: str(x).split()[0]).astype('category')
    ...
    # 已破案的为一类AA, AO, JA, JO,
    y_col = 'Status'
    df[y_col] = df[y_col].map(lambda y: 1 if y=='IC' else 0)
...
if __name__ == "__main__":
    ...
    # print(f"{util.is_one_to_one(df, 'AREA', 'AREA NAME')=}")
    # print(f"{util.is_one_to_one(df, 'Crm Cd', 'Crm Cd Desc')=}")

    # print(f"{util.is_one_to_one(df, 'Premis Cd', 'Premis Desc')=}")
    # col1 = 'Premis Cd'
    # col2 = 'Premis Desc'
    # dfu = df.drop_duplicates([col1, col2])
    # a = dfu[col2].value_counts()
    # print(f'{a[a>1]}')  # RETIRED (DUPLICATE) DO NOT USE THIS CODE
    # print(f'{dfu.loc[dfu[col2] == "RETIRED (DUPLICATE) DO NOT USE THIS CODE", [col1,col2]]=}')

    # print(f"{util.is_one_to_one(df, 'Weapon Used Cd', 'Weapon Desc')=}")
    # col1 = 'Weapon Used Cd'
    # col2 = 'Weapon Desc'
    # dfu = df.drop_duplicates([col1, col2])
    ...
    # print(f'{a[a!=1]=}')
    # print(f'{b[b!=1]=}')
    # df2 = df.loc[df[col1] == 222,:]
    # print(f'{df2.shape=}')
    
    # print(f"{util.is_one_to_one(df, 'Status', 'Status Desc')=}")
    # col1 = 'Status'
    # col2 = 'Status Desc'
    # dfu = df.drop_duplicates([col1, col2])
    # a = dfu.groupby(col1)[col2].count()
    ...
    # print(f'{a[a!=1]=}')
    # print(f'{b[b!=1]=}')
    # df2 = df.loc[df[col2] == 'UNK',[col1, col2]]
    # print(f'{df2.shape=}')
    
    # vc = df['Status'].value_counts()
    # print(f'{vc=}')
    
    # util.find_useless_colum(df,verbose=True)

    df2 = df.loc[:5]
    print(f'{df.loc[:5,["Mocodes first"]]}')    
