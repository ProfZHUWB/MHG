...
def load(data_dir='../../dataset', drop_useless=True, sampling_cfg=(20000,10000,1,1)): 
    
    # 1. read/uncompress data
    with ZipFile(os.path.join(data_dir, 'kaggle/H1BVisa/h1b_kaggle.csv.zip')) as zipfile:
        with zipfile.open('h1b_kaggle.csv') as f:
            df = pd.read_csv(f)
    ...
    cat_cols = ['CASE_STATUS','EMPLOYER_NAME','SOC_NAME',
                'JOB_TITLE','FULL_TIME_POSITION','WORKSITE', ] # YEAR 2011 - 2016 当作cat_var处理
    for col in cat_cols:
    ...
    df = df[~df['CASE_STATUS'].isnull()]  # delete records with unknown target_value
    y_col = 'CERTIFIED'
    df[y_col] = df['CASE_STATUS'].apply(lambda x: 1 if x=='CERTIFIED' else 0)
    df.drop(columns='CASE_STATUS', inplace=True) # delete CASE_STATUS

    # 5. drop_useless
    if drop_useless:
        # a. manually remove useless columns
        df.drop(columns='Unnamed: 0', inplace=True) # 无意义的列
