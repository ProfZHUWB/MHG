...
def sampling_by_class(df, class_col, sampling_cfg):
    '''
    ...
    sampling_cfg : (p_count,n_count,p_seed,n_seed), optional
        Take p_count,n_count from class 1,0 using random seed p_seed,n_seed respectively.
        The default is None, return entire table.
    ...
    '''
    p_count, n_count, p_seed, n_seed = sampling_cfg
    df_positive = df.query(f'{class_col} == 1 ').sample(n=p_count,random_state=p_seed)
    df_negative = df.query(f'{class_col} == 0 ').sample(n=n_count,random_state=n_seed)
